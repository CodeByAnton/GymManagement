    ...
    const fetchClientSchedule = async () => {
        try {
            const [groupTrainingsResponse, personalServicesResponse] = await Promise.all([
                fetch(`/api/v1/client/schedule/group-trainings?page=${currentPage}&size=10`, {
                    headers: {
                        'Authorization': 'Bearer ' + accessToken
                    }
                }),
                fetch(`/api/v1/client/schedule/personal-services?page=${currentPage}&size=10`, {
                    headers: {
                        'Authorization': 'Bearer ' + accessToken
                    }
                })
            ...
            const combinedData = [
                ...
                ...personalServicesData.personalServices.map(service => ({
                    ...
                    description: `Trainer: ${service.trainerEmail}`,
                                            ...
                                            <MDBCardBody>
                                                ...
                                                <MDBCardText className={"text-start"}>
                                                    ...
                                                    {item.serviceType && <><strong>Service type:</strong> {item.serviceType}
                                                        <br /></>}
                                                    {item.passCode && <><strong>Pass code:</strong> {item.passCode}
                                                        <br /></>}
                                                </MDBCardText>
