...
import { APPLICATIONS, RegisterFlow } from "../../../tools/consts";
import { validateEmail } from "../../../tools/functions";
import { useDispatch } from "react-redux";
import { setError, setErrorMessage } from "../../../redux/actions";

const ApplicationForm = ({ setFlow, userData,
                             setEmail, setPassword, setName, setSurname, setBirthDate, setDesiredRole }) => {
    ...
    const nextHandler = async () => {
        if (userData.email.length === 0 || userData.password.length === 0
            || userData.name.length === 0 || userData.surname.length === 0 || userData.birthDate.length === 0 || userData.desiredRole.length === 0) {
                    ...
                    <div className="card bg-light text-dark" style={{ borderRadius: "1rem" }}>
                        <div className="card-body p-5 text-center pb-4">
                            <div className="my-md-4 pb-2">
                                <h2 className="fw-bold mb-2 text-uppercase">Заявка</h2>
                                ...
                                <div className="form-outline form-dark mb-2">
                                    <select onChange={(event) => setDesiredRole(event.target.value)}
                                            id="typeDesiredRoleX" className="form-select form-select-lg">
                            ...
                            <div className={`${isPhone && "mt-3"}`}>
                                <p className="mb-0">Хотите вернуться назад? <a href="#!"
                                                                               onClick={(event) => {
                                                                                   ...
                                                                                   setFlow(RegisterFlow.RegisterForm);
                                                                               }}
